        ...
        async (request, reply) => {
            ...
            await Promise.all(
                trip.participants.map(async (participant) => {
                    ...
                    const participantEmail = participant.email?.toString;
                    const message = await mail.sendMail({
                        from: {
                            name: 'Equipe plann.er',
                            address: 'oi@plann.er',
                        },
                        to: participant.email?.toString(),
                        ...
                        html: `
                            <div style="font-size: 16px; font-family: sans-serif; line-height: 1.6;">
                                <p>Você foi convidado(a) a participar de uma viagem para <strong>${trip.destination}</strong> nas datas de <strong>${formatedStartDate}</strong> até <strong>${formatedEndDate}</strong>.</p>
                                <p></p>
